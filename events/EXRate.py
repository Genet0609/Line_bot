...
def showCurrency(code) -> "JPY":
    ...
    # 銀行現金賣出價格
    sold_cash = "無資料" if currency[1] == "-" else str(float(currency[2]))
    # 銀行即期買入價格
    buying_spot = "無資料" if currency[1] == "-" else str(float(currency[3]))
    # 銀行即期賣出價格
    sold_spot = "無資料" if currency[1] == "-" else str(float(currency[4]))
